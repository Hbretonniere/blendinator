...
nb_epochs = 1
batch_size = 32
lrs = np.zeros(nb_epochs) + 1.e-4 # 1e-4 * np.exp(-0.1 * np.arange(nb_epochs)) #
betas = np.logspace(-5, 0, nb_epochs)
betas = [0.5]
...
''' Create the model '''
PUNet = ProbaUNet((64, 64, 1), ls_dim, list_channels, block_size, last_conv, checkpoint_path, loss='classic')
